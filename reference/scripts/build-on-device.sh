...
EOF
    ...
    log_info "打包部署文件..."
    cd "${OUTPUT_DIR}"
    tar -czf "../feed-control-system-v${VERSION}-arm7.tar.gz" .
    cd - > /dev/null
    ...
    log_success "部署包创建完成"
    echo ""
    echo "  部署包: dist/feed-control-system-v${VERSION}-arm7.tar.gz (${size})"
    echo "  目录: ${OUTPUT_DIR}/"
}
...
部署步骤:
  1. 构建完成后，部署包位于: dist/feed-control-system-v${VERSION}-arm7.tar.gz
  2. 解压到目标目录:
     tar -xzf dist/feed-control-system-v${VERSION}-arm7.tar.gz -C /opt/feed-control-system
  3. 启动应用:
     cd /opt/feed-control-system
     ./start.sh start
EOF
}
