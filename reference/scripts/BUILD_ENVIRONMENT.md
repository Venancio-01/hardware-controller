...
```
dist/feed-control-system-v1.0.0-arm7-plain/
├── app/
...
最终打包为：`dist/feed-control-system-v1.0.0-arm7-plain.tar.gz`
...
```bash
# 方法 1: 使用 scp
scp dist/feed-control-system-v1.0.0-arm7-plain.tar.gz user@orangepi:/home/user/

# 方法 2: 使用 rsync
rsync -avz dist/feed-control-system-v1.0.0-arm7-plain.tar.gz user@orangepi:/home/user/

# 方法 3: 直接在设备上下载（如果有网络）
ssh user@orangepi
wget http://your-server/feed-control-system-v1.0.0-arm7-plain.tar.gz
```
...
# 解压
tar -xzf feed-control-system-v1.0.0-arm7-plain.tar.gz
cd feed-control-system-v1.0.0-arm7-plain
...
创建服务文件：
```bash
sudo nano /etc/systemd/system/feed-control-system.service
```

内容：
```ini
[Unit]
Description=Feed Control System Application
After=network.target

[Service]
Type=simple
User=your-user
WorkingDirectory=/home/your-user/feed-control-system-v1.0.0-arm7-plain
ExecStart=/usr/bin/node /home/your-user/feed-control-system-v1.0.0-arm7-plain/app/loader.js
Restart=on-failure
RestartSec=10
StandardOutput=append:/home/your-user/feed-control-system-v1.0.0-arm7-plain/feed-control-system.log
StandardError=append:/home/your-user/feed-control-system-v1.0.0-arm7-plain/feed-control-system.log
...
sudo systemctl enable feed-control-system.service
sudo systemctl start feed-control-system.service
sudo systemctl status feed-control-system.service
```
...
添加：
```
@reboot cd /home/your-user/feed-control-system-v1.0.0-arm7-plain && ./start.sh start
```
...
- **使用 PM2 管理进程**:
  ```bash
  npm install -g pm2
  pm2 start app/loader.js --name feed-control-system
  pm2 startup
  pm2 save
  ```
...
docker buildx build \
  ...
  -t feed-control-system:arm7-build-plain \
  --load \
  .

# 提取产物（手动）
docker create --name temp-extract feed-control-system:arm7-build-plain /bin/true
docker cp temp-extract:/app ./dist/app
docker rm temp-extract

# 查看 Docker 镜像大小
docker images | grep feed-control-system
