...
# 版本配置
VERSION="1.0.0"
OUTPUT_DIR="dist/feed-control-system-v${VERSION}-arm7-plain"
export NODE_ENV=production
DOCKER_IMAGE="feed-control-system:arm7-build-plain"
...
#!/bin/bash
# Feed Control System 启动脚本（无加密版本）
...
create_archive() {
    ...
    cd dist
    tar -czf "feed-control-system-v${VERSION}-arm7-plain.tar.gz" "feed-control-system-v${VERSION}-arm7-plain"
    cd ..

    # 计算文件大小
    local size=$(du -h "dist/feed-control-system-v${VERSION}-arm7-plain.tar.gz" | cut -f1)

    log_success "打包完成: dist/feed-control-system-v${VERSION}-arm7-plain.tar.gz (${size})"
}

# 清理
clean() {
    log_info "清理构建产物..."
    rm -rf dist/feed-control-system-*-plain
    docker rm -f temp-extract-plain 2>/dev/null || true
    log_success "清理完成"
}
...
main() {
    ...
    echo ""
    echo "================================================"
    echo "  Feed Control System ARM v7 构建脚本（无加密版本）"
    echo "  版本: ${VERSION}"
    echo "  模式: 本地构建 (QEMU)"
    echo "================================================"
    echo ""
    ...
    echo "================================================"
    echo ""
    echo "输出文件: dist/feed-control-system-v${VERSION}-arm7-plain.tar.gz"
    echo ""
    echo "部署步骤:"
    echo "  1. 复制到 OrangePi:"
    echo "     scp dist/feed-control-system-v${VERSION}-arm7-plain.tar.gz orangepi@192.168.110.220:~/"
    echo ""
    echo "  2. 在 OrangePi 上解压并启动:"
    echo "     tar -xzf feed-control-system-v${VERSION}-arm7-plain.tar.gz"
    echo "     cd feed-control-system-v${VERSION}-arm7-plain"
    echo "     ./start.sh"
    echo ""
    echo "  3. 查看状态/日志:"
