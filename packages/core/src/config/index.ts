...
export function getConfigSummary(): Record<string, unknown> {
  return {
    ...
    hardware: {
      ...
      inputs: {
        apply: config.APPLY_SWITCH_INDEX,
        cabinetDoor: config.CABINET_DOOR_SWITCH_INDEX,
        doorJumpSwitch: config.DOOR_LOCK_SWITCH_INDEX,
        keySwitch: config.KEY_SWITCH_INDEX,
        vibrationSwitch: config.VIBRATION_SWITCH_INDEX,
        alarmCancel: config.ALARM_CANCEL_SWITCH_INDEX,
        authCancel: config.AUTH_CANCEL_SWITCH_INDEX,
        authPass: config.AUTH_PASS_SWITCH_INDEX,
      },
      relays: {
        lock: config.DOOR_LOCK_SWITCH_LIGHT_RELAY_INDEX,
        cabinetAlarm: config.ALARM_LIGHT_RELAY_INDEX,
        controlAlarm: config.CONTROL_ALARM_RELAY_INDEX,
        doorLock: config.DOOR_LOCK_RELAY_INDEX,
      },
    },
    logging: {
