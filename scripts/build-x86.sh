...
# 版本配置
VERSION="1.0.0"
OUTPUT_DIR="dist/node-switch-v${VERSION}-x86"
...
create_archive() {
    ...
    cd dist
    tar -czf "node-switch-v${VERSION}-x86.tar.gz" "node-switch-v${VERSION}-x86"
    cd ..

    # 计算文件大小
    local size=$(du -h "dist/node-switch-v${VERSION}-x86.tar.gz" | cut -f1)

    log_success "打包完成: dist/node-switch-v${VERSION}-x86.tar.gz (${size})"
}
...
main() {
    echo ""
    echo "================================================"
    echo "  Node Switch X86 Linux 构建脚本"
    echo "  版本: ${VERSION}"
    echo "================================================"
    echo ""
    ...
    echo "================================================"
    echo ""
    echo "输出文件: dist/node-switch-v${VERSION}-x86.tar.gz"
    echo ""
    echo "部署步骤:"
    echo "  1. 复制到目标服务器:"
    echo "     scp dist/node-switch-v${VERSION}-x86.tar.gz user@server:~/"
    echo ""
    echo "  2. 在服务器上解压并启动:"
    echo "     tar -xzf node-switch-v${VERSION}-x86.tar.gz"
    echo "     cd node-switch-v${VERSION}-x86"
    echo "     ./start.sh"
    echo ""
    echo "  3. 查看状态/日志:"
