...
DOCKER_IMAGE="node-switch:arm7-build"
...
create_archive() {
    ...
    log_success "打包完成: dist/node-switch-v${VERSION}-arm7.tar.gz (${size})"
}
...
main() {
    ...
    echo "部署步骤:"
    echo "  1. 复制到 OrangePi:"
    echo "     scp dist/node-switch-v${VERSION}-arm7.tar.gz user@orangepi:~/"
    echo ""
    echo "  2. 在 OrangePi 上解压并启动:"
    echo "     tar -xzf node-switch-v${VERSION}-arm7.tar.gz"
