...
describe('RelayStatusAggregator - Index Matching Bug Repro', () => {
  ...
  it('should correctly distinguish CH1 and CH13 using hasIndexChanged', () => {
    // Initial state
    aggregator.update('cabinet', { channels: Array(8).fill(false), raw: 'dostatus00000000' });
    aggregator.update('control', { channels: Array(8).fill(false), raw: 'dostatus00000000' });
  ...
  it('should return false if previousCombined is null', () => {
    ...
    aggregator.update('control', { channels: Array(8).fill(false), raw: 'dostatus00000000' });
    const update = aggregator.update('cabinet', { channels: Array(8).fill(false), raw: 'dostatus00000000' });
