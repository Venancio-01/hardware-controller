...
- CO2: 电磁锁 (RELAY_LOCK_INDEX: 2)
- CO4: 柜体报警器 (RELAY_CABINET_ALARM_INDEX: 4)
...
- S_FINISHED: 供弹完毕（柜门关闭后自动进入，然后归一到 S_IDLE）
...
#### 授权通过
- ACT_VOICE_AUTH_PASS_CABINET: "授权通过，已开锁请打开柜门"（柜体端）
- ACT_VOICE_AUTH_PASS_CONTROL: "授权通过，已开锁"（控制端）

#### 柜门打开
- ACT_VOICE_DOOR_OPEN_CONTROL: "柜门已打开"（控制端）
- ACT_VOICE_DOOR_OPEN_CABINET: "已开门，请取弹，取弹后关闭柜门，并复位按键"（柜体端）
...
控制端用户同意授权 -> 柜体端播报【授权通过，已开锁请打开柜门】
                     + 控制端播报【授权通过，已开锁】

柜体端打开柜门 -> 控制端播报【柜门已打开】
                 + 柜体端播报【已开门，请取弹，取弹后关闭柜门，并复位按键】

柜体端用户供弹结束关闭柜门 -> 控制端【授权申请灯】灭
                            + 柜体端【电磁锁】落锁
                            + 控制端播报【供弹完毕】
                            + 柜体端播报【供弹完毕】
```
...
- S_AUTHORIZED --EVT_DOOR_OPEN--> S_DOOR_OPEN / ACT_VOICE_DOOR_OPEN_CONTROL + ACT_VOICE_DOOR_OPEN_CABINET
- S_DOOR_OPEN --EVT_DOOR_CLOSE--> S_FINISHED / ACT_LOCK + ACT_APPLY_LIGHT_OFF + ACT_VOICE_FINISH
...
**状态迁移：**
- S_APPLYING --EVT_REFUSE--> S_IDLE / ACT_APPLY_LIGHT_OFF + ACT_VOICE_REFUSE

### 钥匙开门报警
- ANY --EVT_KEY_TRIGGER--> S_KEY_ALARM / ACT_CABINET_ALARM_ON + ACT_CONTROL_ALARM_ON + ACT_VOICE_KEY_ALARM
...
### 振动报警
- ANY --EVT_VIBRATION--> S_VIBRATION_ALARM / ACT_CABINET_ALARM_ON + ACT_CONTROL_ALARM_ON + ACT_VOICE_VIB_ALARM
...
- S_MONITOR_ALARM --EVT_ALARM_CANCEL--> S_IDLE / ACT_CONTROL_ALARM_OFF + ACT_VOICE_ALARM_CANCEL
...
stateDiagram-v2
    ...
    S_APPLYING --> S_KEY_ALARM: EVT_KEY_TRIGGER
    S_AUTHORIZED --> S_KEY_ALARM: EVT_KEY_TRIGGER
    S_DOOR_OPEN --> S_KEY_ALARM: EVT_KEY_TRIGGER

    S_IDLE --> S_VIBRATION_ALARM: EVT_VIBRATION
    S_APPLYING --> S_VIBRATION_ALARM: EVT_VIBRATION
    S_AUTHORIZED --> S_VIBRATION_ALARM: EVT_VIBRATION
    S_DOOR_OPEN --> S_VIBRATION_ALARM: EVT_VIBRATION
